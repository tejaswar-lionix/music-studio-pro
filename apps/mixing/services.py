"""Services for mixing - Stem mixing - gain, pan, EQ, compression, reverb per stem"""
import re, hashlib, json, time
from typing import Dict, Any


def mixing_gain_0(gain):
    """Gain 0 distinct per stem"""
    return min(0.0, gain + -1)

def mixing_gain_1(gain):
    """Gain 1 distinct per stem"""
    return min(0.0, gain + 0)

def mixing_gain_2(gain):
    """Gain 2 distinct per stem"""
    return min(0.0, gain + 1)

def mixing_gain_3(gain):
    """Gain 3 distinct per stem"""
    return min(0.0, gain + -1)

def mixing_gain_4(gain):
    """Gain 4 distinct per stem"""
    return min(0.0, gain + 0)

def mixing_gain_5(gain):
    """Gain 5 distinct per stem"""
    return min(0.0, gain + 1)

def mixing_gain_6(gain):
    """Gain 6 distinct per stem"""
    return min(0.0, gain + -1)

def mixing_gain_7(gain):
    """Gain 7 distinct per stem"""
    return min(0.0, gain + 0)

def mixing_gain_8(gain):
    """Gain 8 distinct per stem"""
    return min(0.0, gain + 1)

def mixing_gain_9(gain):
    """Gain 9 distinct per stem"""
    return min(0.0, gain + -1)

def mixing_gain_10(gain):
    """Gain 10 distinct per stem"""
    return min(0.0, gain + 0)

def mixing_gain_11(gain):
    """Gain 11 distinct per stem"""
    return min(0.0, gain + 1)

def mixing_gain_12(gain):
    """Gain 12 distinct per stem"""
    return min(0.0, gain + -1)

def mixing_gain_13(gain):
    """Gain 13 distinct per stem"""
    return min(0.0, gain + 0)

def mixing_gain_14(gain):
    """Gain 14 distinct per stem"""
    return min(0.0, gain + 1)

def mixing_gain_15(gain):
    """Gain 15 distinct per stem"""
    return min(0.0, gain + -1)

def mixing_gain_16(gain):
    """Gain 16 distinct per stem"""
    return min(0.0, gain + 0)

def mixing_gain_17(gain):
    """Gain 17 distinct per stem"""
    return min(0.0, gain + 1)

def mixing_gain_18(gain):
    """Gain 18 distinct per stem"""
    return min(0.0, gain + -1)

def mixing_gain_19(gain):
    """Gain 19 distinct per stem"""
    return min(0.0, gain + 0)

def mixing_gain_20(gain):
    """Gain 20 distinct per stem"""
    return min(0.0, gain + 1)

def mixing_gain_21(gain):
    """Gain 21 distinct per stem"""
    return min(0.0, gain + -1)

def mixing_gain_22(gain):
    """Gain 22 distinct per stem"""
    return min(0.0, gain + 0)

def mixing_gain_23(gain):
    """Gain 23 distinct per stem"""
    return min(0.0, gain + 1)

def mixing_gain_24(gain):
    """Gain 24 distinct per stem"""
    return min(0.0, gain + -1)
